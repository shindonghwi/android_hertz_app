...
import mago.apps.hertz.ui.screens.answer.detail.model.AnswerPatchData
...
class AnswerDetailViewModel @Inject constructor(
    ...
    private fun initPathAnswerData(data: Answer?) {
        answerPatchData = AnswerPatchData(
            ...
            anger = data?.voice?.emotionList?.filter { it.type == EmotionType.ANGRY.name }
    ...
    fun updatePatchAnswerTag(tags: List<String>) {
    ...
    fun updatePatchAnswerEmotion(type: String, rate: Int) {
        when (type) {
            ...
            EmotionType.ANGRY.name -> answerPatchData?.anger = rate
        ...
        answerPatchData?.let {
            if (it.answerSeq == 0) return TOAST_CODE_QUESTION_1
                ...
                patchAnswerUseCase(
                    ...
                    it.anger, it.neutral, it.happiness, it.sadness
                ).onEach {
                    when (it) {
                        is Resource.Loading -> {
